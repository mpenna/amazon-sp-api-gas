...
const request = require('./gas-request');
...
class SellingPartner {
  ...
  constructor(config) {
    // const logObj = {
    //   function: 'SellingPartner@constructor',
    //   params: { config }
    // }
    ...
    this._options = Object.assign({
      ...
      // user_agent: HtmlService.getUserAgent(),
      debug_log: false,
      timeouts: {}
    }, config.options);
  ...
  _validateEndpointsVersions(endpoints_versions) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateEndpointsVersions',
        params: { endpoints_versions }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _validateEncryptionDetails(details) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateEncryptionDetails',
        params: { details }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _validateUpOrDownloadSuccess(res, request_type) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateUpOrDownloadSuccess',
        params: { res, request_type }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _decryptBuffer(details, chunks) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_decryptBuffer',
        params: { details, chunks }
      }
      Logger.log(JSON.stringify(logObj));
    }

    if (details.encryptionDetails) {
      // Logger.log('SellingPartner@_decryptBuffer before Buffer.concat');
      let encrypted_buffer = Buffer.concat(chunks);

      // Logger.log('SellingPartner@_decryptBuffer before crytpo.createDecipheriv');
      ...
      // Logger.log('SellingPartner@_decryptBuffer before second Buffer.concat');
      return Buffer.concat([decipher.update(encrypted_buffer), decipher.final()]);
    }

    // Logger.log('SellingPartner@_decryptBuffer before third Buffer.concat', { chunks });

    // return Buffer.concat(chunks);

    return Buffer.concat([Buffer.from(chunks)]);

    // const typedArray = [];
    // for (var i = 0; i < chunks.length; i++) {
    //   typedArray.push(chunks[i] < 0 ? chunks[i] + 256 : chunks[i]);
    // }
    // // return Buffer.concat(new Buffer(typedArray));
    // return Buffer.concat(typedArray);

    // const bufferArray = new Buffer(chunks)
    // return Buffer.concat(bufferArray.buffer.);

    // const b = Buffer.from(chunks);
    // const typedArray = new Buffer(chunks);

    // return Buffer.concat(new Buffer(chunks));
  }
  // _decryptBuffer(details, chunks) {
  //   if (details.encryptionDetails) {
  //     let encrypted_buffer = Buffer.concat(chunks);
  ...
  _decodeBuffer(decrypted_buffer, headers, charset) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_decodeBuffer',
        params: { decrypted_buffer, headers, charset }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _constructRefreshAccessTokenBody(scope) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_constructRefreshAccessTokenBody',
        params: { scope }
      }
      Logger.log(JSON.stringify(logObj));
    }
    ...
    if (scope) {
      ...
      // Logger.log('grant type = refresh_token');

      body.grant_type = 'refresh_token';
      body.refresh_token = this._refresh_token;
    } else {
      // Logger.log('no scope provided');
  ...
  _tokenExists(scope) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_tokenExists',
        params: { scope }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  async _validateAccessTokenAndRoleCredentials(scope) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateAccessTokenAndRoleCredentials',
        params: { scope }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _validateMethod(method) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateMethod',
        params: { method }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _validateOperationAndEndpoint(operation, endpoint) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateOperationAndEndpoint',
        params: { operation, endpoint }
      }
      Logger.log(JSON.stringify(logObj));
    }
    ...
    if (!operations[operation]) {
      // Logger.log('operation not found');
    ...
    if (!endpoints[endpoint].__operations.includes(operation)) {
      // Logger.log('invalid operatiom for endpoint');
  ...
  _getFallbackVersion(operation, endpoint, version) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_getFallbackVersion',
        params: { operation, endpoint, version }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _validateLocallySetVersion(operation, endpoint, version) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateLocallySetVersion',
        params: { operation, endpoint, version }
      }
      Logger.log(JSON.stringify(logObj));
    }
    ...
    if (!endpoints[endpoint].__versions.includes(version)) {
      // Logger.log('explicitly specified version in .callAPI can\'t be found for the endpoint');
    ...
    if (!endpoints[endpoint][version][operation]) {
      // Logger.log('operation is not supported for the version');
  ...
  _validateGloballySetVersion(operation, endpoint) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateGloballySetVersion',
        params: { operation, endpoint }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _validateAndGetVersion(operation, endpoint, version) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateAndGetVersion',
        params: { operation, endpoint, version }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _validateOperationAllowance(scope) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_validateOperationAllowance',
        params: { scope }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  _constructExchangeBody(auth_code) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_constructExchangeBody',
        params: { auth_code }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  async _createReport(req_params) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_createReport',
        params: { req_params }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  async _getReport(req_params, report_id) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_getReport',
        params: { req_params, report_id }
      }
      Logger.log(JSON.stringify(logObj));
    }
    ...
    if (res.processingStatus === 'DONE') {
      ...
      if (this._options.debug_log) {
        Logger.log('Current status of report ' + req_params.body.reportType + ': ' + res.processingStatus);
      }
  ...
  async _getReportDocument(req_params, report_document_id) {
    if (this._options.debug_log) {
      const logObj = {
        function: 'SellingPartner@_getReportDocument',
        params: { req_params, report_document_id }
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  async refreshRoleCredentials() {
    if (this._options.debug_log) {
      const logObj = {
        function: 'refreshRoleCredentials',
        params: {}
      }
      Logger.log(JSON.stringify(logObj));
    }
  ...
  async callAPI(req_params) {
    ...
    if (req_params.api_path) {
      ...
      // Logger.log('building req_params');
      ...
      // Logger.log(JSON.stringify(req_params));
      ...
      if (res.statusCode === 403) {
        if (error.code === 'Unauthorized' && this._options.auto_request_tokens) {
          if (/access token.*expired/.test(error.details)) {
            if (this._options.debug_log) {
              Logger.log('Access token expired, refreshing it now');
            }
            ...
            if (this._options.debug_log) {
              Logger.log('Role credentials security token expired, refreshing it now');
            }
        ...
        if (res.headers['x-amzn-ratelimit-limit'] || req_params.restore_rate) {
          ...
          if (this._options.debug_log) {
            Logger.log('Request throttled, retrying a call of "' + (req_params.operation || req_params.api_path) + '" in ' + restore_rate + ' seconds...');
          }
